...
class DashboardController extends Controller
{
    ...
    public function mgbkIndex()
    {
        /** @var \App\Models\User $user */
        $user = Auth::user();

        /* ── Origins milik MGBK ini ── */
        $myOriginIds = Origin::where('mgbk_id', $user->id)->pluck('id');
        $myOrigins   = Origin::where('mgbk_id', $user->id)->get();

        /* ── Base query — hanya hasil dari institusi milik MGBK ── */
        $baseQ = Result::query()->whereIn('origin_id', $myOriginIds);
        ...
        /* ── Pengerjaan per Institusi milik MGBK ── */
        ...
        return Inertia::render('Mgbk/Dashboard', [
            'title'                  => 'Dashboard',
            'description'            => 'Ringkasan data kuisioner institusi yang Anda kelola',
            'summary'                => [
                'total_results'           => $totalResults,
                'total_participants'      => $totalParticipants,
                'avg_gus'                 => $avgGus,
