...
/* ── Extended type for MGBK (adds per_origin) ── */
type PerOrigin = { name: string; total: number };
type MgbkReportPrintProps = ReportPrintProps & { per_origin: PerOrigin[] };

const MgbkReportPrint = ({
    filters,
    summary,
    ...
    const hBarOptions: ApexOptions = {
        ...
        plotOptions: {
            bar: { horizontal: true, borderRadius: 3, barHeight: "55%" },
        },
        colors: CHART_SOLID,
        dataLabels: { enabled: true, style: { fontSize: "9px" } },
        xaxis: {
            categories: per_origin.map((o: PerOrigin) => o.name),
            labels: { style: { fontSize: "9px", colors: T.muted } },
        },
        yaxis: { labels: { style: { fontSize: "9px", colors: T.muted } } },
        grid: { borderColor: T.line },
    ...
    return (
        <div style={wrap}>
            ...
            >
                ...
                {/* Per Institusi — MGBK specific */}
                <ChartBox title="Pengerjaan per Institusi" border={T.border}>
                    {per_origin.length > 0 ? (
                        <ReactApexChart
                            type="bar"
                            height={190}
...
export default MgbkReportPrint;
